/**********************************************************
 * 本声明须出现于每次作业自述文件readme.txt的首部。否则，
 * 你的作业将被视作违反了Honor Code规则，并将会按照本课程
 * 约定的方式进行处理。
 **********************************************************/

/***********************************************************
 * 我真诚地保证：
 *
 * 我独立地完成了本程序从分析、设计到编码的所有工作。如果
 * 在上述过程中我曾因遇到困难而求教于人，那么我均已在报告
 * 中详细地列举了我所遇到的问题以及别人给我的提示。
 *
 * 在此，我感谢XXX、...XXX对我的启发和帮助。在下面的报告中，
 * 我还会具体地提到他们对我的帮助之处。
 *
 * 我的程序中凡引用到其它程序或文档之处，无论是来自教材、
 * 课堂笔记、网络还是其它参考资料，我都已在程序注释中明确
 * 注明。我也清楚地知道，因这样做可能降低了作业难度，可能
 * 影响起评分。
 *
 * 我从未抄袭或盗用别人的程序，不管是原封不动地复制，还是
 * 经过了某种（些）等价转换。
 *
 * 我编写这个程序的目的，不是在于故意破坏或妨碍其它计算机
 * 系统的正常运转。
 *
 * 李天阳
 * 2009011419
 * 2011-02-22
 **********************************************************/
 
/***********************************************************
 * 本题算法设计思路来源于 CLRS 中的 Problem 32-1. 使用 KMP 算法中计算 prefix (next)
 * 函数后用递推计算 repetition factor
 * 易得本算法的复杂度为 O(n)
 * 算法正确性的详细证明可以参见参考文献中的 paper
 **********************************************************/

/***********************************************************
 * 参考文献
 * [1] Donald Knuth; James H. Morris, Jr, Vaughan Pratt (1977). 
 * "Fast pattern matching in strings". 
 * SIAM Journal on Computing 6 (2): 323–350. doi:10.1137/0206024
 * [2] GALIL Z., SEIFERAS J., 1983, 
 * Time-space optimal string matching, 
 * Journal of Computer and System Science 26(3):280-294.
 * [3] Cormen, Thomas H.; Leiserson, Charles E.; Rivest, Ronald L.; Stein, Clifford (2009). 
 * Introduction to Algorithms (3rd ed.). 
 * MIT Press. ISBN 0-262-03384-4.
 **********************************************************/



