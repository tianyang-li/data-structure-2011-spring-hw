...
public:
	...
	public:
		...
		CircList<typename AdjListGraph<T>::Vertex::Vertex *> adj_list;  // adjacency list
		T data;
...
inline void AdjListGraph<T>::NoEdgeDFS(VertexPtr start, NoEdgeDFSProcVert1 &proc1, NoEdgeDFSProcVert2 &proc2
	...
	ListNode<VertexPtr> *cur_nb = start->adj_list.GetHead();
	while (NULL != cur_nb) {
		if (false == cur_nb->data_->flag) {
			proc2.Proc(start, from_vert, cur_nb->data_);
			this->NoEdgeDFS(cur_nb->data_, proc1, proc2, proc3, proc4, start);
			proc3.Proc(start, from_vert, cur_nb->data_);
		}
